...
//String P005GS = "\x5E\x50\x30\x30\x35\x47\x53\x58\x14\x0D"; // Aici Complet dar Sistemul calculeaza automat CRC si pune CR
String P005GS = "\x5E\x50\x30\x30\x35\x47\x53"; // General Status Query
String P006MOD = "\x5E\x50\x30\x30\x36\x4D\x4F\x44"; // Working Mode
String P005FWS = "\x5E\x50\x30\x30\x35\x46\x57\x53"; // Faults
QRaw _qRaw;
//QAv _qAv;
...
bool onPIGS() //QPIGS<cr>: Device general status parameters inquiry
{
  ...
  else
  {
    ...
    int index = 5; //after the starting '^D106'
      _qpigsMessage.gridV = (short)getNextLong(_commandBuffer, index);               //1
      _qpigsMessage.gridHz = (short)getNextLong(_commandBuffer, index);              //2
      _qpigsMessage.acOutV = (short)getNextLong(_commandBuffer, index);              //3
      _qpigsMessage.acOutHz = (short)getNextLong(_commandBuffer, index);             //4
      ...
      _qpigsMessage.battV = (short)getNextLong(_commandBuffer, index);               //8
      _qpigsMessage.battV_SCC = (short)getNextLong(_commandBuffer, index);           //9    
      _qpigsMessage.battV_SCC_2 = (short)getNextLong(_commandBuffer, index);         //10
      ...
      _qpigsMessage.solarV_PV1 = (short)getNextLong(_commandBuffer, index);          //19
      _qpigsMessage.solarV_PV2 = (short)getNextLong(_commandBuffer, index);          //20
